...
- 必须且只能调用 Cursor AI 预设的 `/skill git-submit` 技能。
...
## 3. Changelog 时间戳规则
- 标题格式：模块_YYYY_MM_DD_HHmm（例如：`后端_2026_03_31_1454`）。
...
## 8. 前端架构与渲染纪律 (Next.js)
- **环境约束**: 必须基于 Node.js 18.17.0+ 环境, 且**统一使用 pnpm 8.0+** 作为包管理器, 严禁使用 npm/yarn.
- 前端开发必须严格遵守 App Router 规范，默认使用 Server Components (RSC) 以优化性能。
- 仅在需要状态 (Hooks) 或浏览器交互时，才在文件顶部显式声明 `"use client"`，禁止滥用导致全局退化为客户端渲染。
- 必须使用 TypeScript 进行开发，严禁使用 `any` 类型，必须定义清晰的 `interface` 或 `type`。
- 样式开发强制使用 **Tailwind CSS v4**。
- 前端代码编写或审查时，必须使用 `/skill frontend-next-code-check-wuhao` 进行规范检查。
