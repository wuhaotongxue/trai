...
- 启动服务前，**必须先检查 README 中的环境配置说明**
- 如果 README 指定了特定环境名称（如 `trai31313`），必须使用该名称
- 如果该环境不存在，**必须先创建**（如 `conda create -n trai31313 python=3.13.13 -y`），禁止跳过此步骤直接使用其他环境

#### 依赖安装规范

- 安装依赖时使用 conda 环境中的 python：`D:\anaconda3\envs\{环境名}\python.exe`
- 安装命令：`conda run -n {环境名} pip install -i https://pypi.tuna.tsinghua.edu.cn/simple -e .`
