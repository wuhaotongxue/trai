...
- **不要把所有图片集中放在「这次更新了什么」一个区域**，用户体验差、节奏感差。
- **每张图片放到对应章节内部**，紧跟在相关内容段落之后，让图片成为叙事的辅助说明，而非孤立的图册。
- 单章节 1 张图：直接放在小节内容之后。
- 单章节多张图：用 `display:grid` 布局（2-3 列），保持与文字的关联性。
- 图片说明文字（alt）要具体，避免「截图」「图片」等模糊描述。
...
- [ ] 文档末尾已补充联系方式块，格式：联系邮箱 wuhaotongxue@gmail.com。
