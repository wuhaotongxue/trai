...
files.forEach(file => {
  ...
  content = content.replace(/bg-card\/[0-9]+/g, 'bg-white dark:bg-slate-900 border-2 border-slate-900 dark:border-white');
  content = content.replace(/bg-background\/[0-9]+/g, 'bg-white dark:bg-slate-900');
  
  content = content.replace(/bg-gradient-to-[a-z]+ from-[a-z0-9-]+ to-[a-z0-9-]+/g, 'bg-amber-400');
  
  content = content.replace(/text-muted-foreground/g, 'text-slate-900 dark:text-white font-bold');
