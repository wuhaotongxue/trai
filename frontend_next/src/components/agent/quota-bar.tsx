...
export function QuotaBar() {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-1.5 rounded-full bg-slate-100/60 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60 text-xs">
      ...
      {toolQuota.unlimited ? (
        <span className="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-semibold text-[10px]">无限</span>
      ) : (
        <>
          ...
          <span className="px-1.5 py-0.5 rounded bg-slate-200/80 dark:bg-slate-700/80 text-muted-foreground text-[10px]">{toolQuota.billing_month}</span>
        </>
      )}
    </div>
  );
