...
export default function QuotasPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-slate-900">配额配置</h1>
        <p className="text-sm text-slate-500 mt-0.5">管理各套餐的调用额度与服务限制</p>
      </div>
          ...
          >
            ...
            <CardHeader className="pb-2">
              <div className="flex items-start justify-between">
                ...
                {plan.popular && (
                  <span className="text-xs bg-blue-100 text-blue-600 font-medium px-2 py-0.5 rounded-full">热门</span>
                )}
              </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-semibold text-slate-900">快速配置</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-xs text-slate-500">修改默认全局配额限制，影响所有未自定义的用户</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label className="text-xs font-medium text-slate-600">Agent 调用上限</Label>
              <Input className="h-9 rounded-lg" defaultValue="50" />
              <p className="text-xs text-slate-400">次/用户/月</p>
            </div>
            <div className="space-y-2">
              <Label className="text-xs font-medium text-slate-600">每日会话上限</Label>
              <Input className="h-9 rounded-lg" defaultValue="20" />
              <p className="text-xs text-slate-400">-1 表示不限制</p>
            </div>
            <div className="space-y-2">
              <Label className="text-xs font-medium text-slate-600">单会话消息数</Label>
              <Input className="h-9 rounded-lg" defaultValue="10" />
              <p className="text-xs text-slate-400">轮次限制</p>
            </div>
          </div>
