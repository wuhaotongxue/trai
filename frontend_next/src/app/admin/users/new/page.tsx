  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <Link href="/admin/users" className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors">
          <ArrowLeft className="h-4 w-4" />
          返回用户列表
        </Link>
        <span className="text-slate-300">/</span>
        <span className="text-sm text-slate-900 font-medium">新增用户</span>
      </div>

      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900">新增用户</h1>
          <p className="text-sm text-slate-500 mt-0.5">手动添加新用户账号</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {errors.password ? (
                  ...
                  <p className="text-xs text-slate-400">密码将加密存储，首次登录后建议用户立即修改</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold text-slate-900">套餐配置</CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <p className={`text-sm font-bold ${
                        form.plan === plan.value
                          ? plan.value === "free" ? "text-slate-700" : plan.value === "pro" ? "text-blue-600" : "text-amber-600"
                          : "text-slate-600"
                      }`}>{plan.label}</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/admin/users">
              <Button variant="outline" size="sm" className="h-10 px-6 rounded-lg border-slate-200">
                取消
              </Button>
            </Link>
        ...
        <div className="space-y-4">
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-slate-900">创建说明</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-xs text-slate-500 leading-relaxed">
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-center font-bold">1</div>
                <p>用户创建后会收到一封激活邮件，需点击链接激活账号</p>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-center font-bold">2</div>
                <p>初始密码由管理员设定，建议用户在首次登录后立即修改</p>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded bg-blue-50 text-blue-600 flex items-center justify-center flex-shrink-0 mt-0.5 text-center font-bold">3</div>
                <p>套餐配额按自然月统计，月末重置</p>
              </div>
            </CardContent>
          </Card>
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="space-y-2">
              {[
                ...
                <button
                  key={item.label}
                  className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg bg-slate-50 hover:bg-slate-100 text-xs font-medium text-slate-600 transition-colors text-left"
                >
