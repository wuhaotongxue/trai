...
const statusInfo = {
  ok: { cls: "text-emerald-400", bg: "bg-emerald-500/15", icon: CheckCircle2, label: "正常" },
  warn: { cls: "text-amber-400", bg: "bg-amber-500/15", icon: AlertCircle, label: "响应慢" },
  error: { cls: "text-red-400", bg: "bg-red-500/15", icon: XCircle, label: "故障" },
};
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-foreground">网络状态</h1>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "正常", value: okCount, total: endpoints.length, color: "bg-emerald-500", text: "text-emerald-400", bg: "bg-emerald-500/15", icon: CheckCircle2 },
          { label: "警告", value: warnCount, total: endpoints.length, color: "bg-amber-500", text: "text-amber-400", bg: "bg-amber-500/15", icon: AlertCircle },
          { label: "故障", value: errorCount, total: endpoints.length, color: "bg-red-500", text: "text-red-400", bg: "bg-red-500/15", icon: XCircle },
        ].map((item) => (
          <Card key={item.label} className="border-0 shadow-sm">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold text-foreground">{item.value}/{item.total}</p>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold text-foreground">外网端点连通性</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-xs font-medium text-foreground/90 truncate">{ep.name}</p>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{ep.lastCheck}</p>
                  </div>
                </div>
              );
            })}
