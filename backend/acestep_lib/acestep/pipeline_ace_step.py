...
class ACEStepPipeline:
    ...
    def save_wav_file(
        ...
        # torchaudio tensor: (channels, samples) -> soundfile: (samples, channels)
        if target_wav.dim() == 2:
            target_wav = target_wav.T
        logger.info(f"Saving audio to {output_path_wav} using soundfile")
        sf.write(output_path_wav, target_wav.cpu().numpy(), sample_rate)
    ...
    def __call__(
        ...
        # 先解码获取音频数据（使用 CpuOffloader 处理设备转换）
        offloader = CpuOffloader(self.music_dcae, self.device)
        with torch.no_grad():
            with offloader:
                if self.overlapped_decode and audio_duration > 48:
                    _, pred_wavs = self.music_dcae.decode_overlap(target_latents, sr=48000)
                else:
                    _, pred_wavs = self.music_dcae.decode(target_latents, sr=48000)
        pred_wavs = [pred_wav.cpu().float() for pred_wav in pred_wavs]

        # 保存音频文件
        output_audio_paths = []
        for i, pred_wav in enumerate(pred_wavs):
            output_audio_path = self.save_wav_file(
                pred_wav,
                i,
                save_path=save_path,
        ...
        # Return audio data directly instead of file paths
        class AudioResult:
            pass
        result = AudioResult()
        result.audios = pred_wavs  # Return audio tensors
        result.sample_rate = 48000
        result.file_paths = output_audio_paths  # Also include file paths
        return result
