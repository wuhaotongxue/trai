...
def extract_version_from_filename(filename):
    """从文件名提取版本号，如 'TRAI_0.0.1.202604251200.exe' -> '0.0.1.202604251200'"""
...
if version:
    exe_file = Path(__file__).parent.parent.parent / "client_electron" / "release" / f"TRAI_{version}.exe"

# 如果没指定版本或文件不存在，自动查找最新的 exe
if not exe_file or not exe_file.exists():
    release_dir = Path(__file__).parent.parent.parent / "client_electron" / "release"
    exe_files = sorted(release_dir.glob("TRAI_*.exe"), key=lambda f: f.stat().st_mtime, reverse=True)
    if exe_files:
