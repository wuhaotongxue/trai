...
async def submit_feedback(request: FeedbackRequest, user: dict[str, Any] = Depends(get_current_user)) -> Any:
    """提交一个用户反馈
...
async def list_feedbacks(user: dict[str, Any] = Depends(get_current_user)) -> Any:
    """获取当前用户提交的所有反馈
