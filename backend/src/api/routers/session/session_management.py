    ...
    async def export_session(
        ...
        try:
            ...
            if len(content_bytes) > self.MAX_EXPORT_SIZE:
                raise HTTPException(
                    status_code=400,
                    detail={"code": 400, "message": f"Session too large for export (max {self.MAX_EXPORT_SIZE // (1024*1024)}MB)"},
                )
            
            logger.info(
                f"Session exported | session_id={session_id} | "
                f"format={format.value} | size={len(content_bytes)} bytes"
            )
