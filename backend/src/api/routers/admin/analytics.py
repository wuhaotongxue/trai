...
async def get_wecom_analytics(admin: AdminUser) -> WeComAnalyticsResponse:
    ...
    for d in depts:
        count = (
            db.scalar(
                select(func.count(UserDepartmentMappingModel.t_user_id)).where(
        ...
        new_hires = (
            db.scalar(
                ...
                .where(UserDepartmentMappingModel.t_dept_id == d.t_dept_id, UserModel.t_created_at >= week_ago)
            )
            or 0
        )
        ...
        resignations = (
            db.scalar(
                select(func.count(UserModel.t_user_id))
                .join(UserDepartmentMappingModel, UserModel.t_user_id == UserDepartmentMappingModel.t_user_id)
                .where(UserDepartmentMappingModel.t_dept_id == d.t_dept_id, UserModel.t_deleted_at >= week_ago)
            )
            or 0
        )

        dept_stats.append(
            WeComDeptStats(dept_name=d.t_name, user_count=count, new_hires=new_hires, resignations=resignations)
        )
    ...
    for i in range(4):
        ...
        hires = (
            db.scalar(select(func.count(UserModel.t_user_id)).where(UserModel.t_created_at.between(start, end))) or 0
        )
        resigns = (
            db.scalar(select(func.count(UserModel.t_user_id)).where(UserModel.t_deleted_at.between(start, end))) or 0
        )
        weekly_trend.append({"week": f"W-{i}", "hires": hires, "resignations": resigns})

    return WeComAnalyticsResponse(
        total_users=total_users, total_depts=total_depts, dept_stats=dept_stats, weekly_trend=weekly_trend[::-1]
    )
