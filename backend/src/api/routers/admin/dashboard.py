...
async def get_dashboard(admin: AdminUser) -> DashboardResponse:
    ...
    vip_users = db.execute(select(func.count()).where(UserModel.t_role == "vip")).scalar() or 0
    total_sessions = db.execute(select(func.count()).select_from(ChatSessionModel)).scalar() or 0
    total_messages = db.execute(select(func.count()).select_from(MessageModel)).scalar() or 0

    new_this_month = (
        db.execute(
            select(func.count()).where(UserModel.t_created_at >= datetime.strptime(month_start, "%Y-%m-%d"))
        ).scalar()
        or 0
    )
