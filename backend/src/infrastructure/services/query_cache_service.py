...
class QueryCacheService:
    ...
    def __init__(
        ...
        logger.info(f"QueryCacheService initialized | ttl={default_ttl}s | max_size={max_size} | enabled={enabled}")
    ...
    def _evict_oldest(self) -> None:
        ...
        oldest_key = min(self._cache_store.keys(), key=lambda k: self._cache_store[k]["created_at"])
    ...
    def invalidate_pattern(self, pattern: str) -> int:
        ...
        Returns:
            使失效的条目数
        """
        keys_to_delete = [k for k in self._cache_store.keys() if pattern in k]
