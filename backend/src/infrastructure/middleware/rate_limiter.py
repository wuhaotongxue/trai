...
class RateLimiter:
    ...
    def _check_sliding_window(
        ...
        self._request_history[key] = [
            t for t in self._request_history[key] if t > window_start
        ]
    ...
    def _detect_abnormal_traffic(self, ip: str) -> bool:
        ...
        if len(recent_requests) > self.BAN_CONFIG["threshold_requests_per_second"]:
            logger.warning(
                f"Abnormal traffic detected | ip={ip} | "
                f"requests_in_1s={len(recent_requests)}"
            )
    ...
    def _ban_ip(self, ip: str) -> None:
        ...
        logger.warning(
            f"IP banned | ip={ip} | duration={self.BAN_CONFIG['ban_duration_seconds']}s"
        )
    ...
    async def check_rate_limit(
        ...
        response.headers["X-RateLimit-Remaining"] = str(max(0, self.DEFAULT_LIMITS["global"]["requests_per_minute"] - count))
