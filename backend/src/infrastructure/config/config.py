...
class AppConfig:
    ...
    def _load_from_environment(self) -> None:
        ...
        self.database.echo_sql = os.getenv("DB_ECHO_SQL", "").lower() in ("true", "1")
        ...
        # AI服务配置
        self.ai_service.provider = os.getenv("AI_PROVIDER", self.ai_service.provider)
        self.ai_service.api_key = os.getenv(
            "OPENAI_API_KEY",
            "",  # 兼容旧变量名
        ) or os.getenv("AI_API_KEY", "")
        self.ai_service.base_url = os.getenv("AI_BASE_URL", self.ai_service.base_url) or os.getenv(
            "OPENAI_BASE_URL", ""
        )
        self.ai_service.default_model = os.getenv("AI_DEFAULT_MODEL", self.ai_service.default_model)

        # 缓存配置
        self.cache.enabled = os.getenv("CACHE_ENABLED", "").lower() in ("true", "1")
        self.cache.default_ttl = int(os.getenv("CACHE_TTL", str(self.cache.default_ttl)))

        # 限流配置
        self.rate_limit.enabled = os.getenv("RATE_LIMIT_ENABLED", "").lower() in ("true", "1")
        self.rate_limit.global_rpm = int(os.getenv("RATE_LIMIT_GLOBAL_RPM", str(self.rate_limit.global_rpm)))

        # 日志配置
        self.logging.level = os.getenv("LOG_LEVEL", self.logging.level)
        self.logging.json_format = os.getenv("LOG_JSON_FORMAT", "").lower() in ("true", "1")
        self.logging.file_output = os.getenv("LOG_FILE_OUTPUT", "").lower() in ("true", "1")
    ...
    def __repr__(self) -> str:
        return f"AppConfig(environment={self.environment.value}, host={self.host}:{self.port})"
