...
class MediaNotifier:
    ...
    def _build_markdown(self, media_type: str, prompt: str, file_url: str, duration: float = 0.0) -> str:
        """构建地理专家风格的 Markdown 通知内容"""
        ...
        content = (
            ...
            f"作为地理专家，我刚刚观测到了这一全新生成的数字奇观。它的板块构造非常完美，请您查收这片新大陆的勘探结果：\n\n"
            f"👉 **[点击查看生成结果]({file_url})**\n\n"
            f"*“地质的运动造就了山川，而 AI 的算力造就了这绝妙的作品。”*"
        )
        return content

    def notify(self, media_type: str, prompt: str, file_url: str, duration: float = 0.0) -> bool:
        """发送通知"""
        content = self._build_markdown(media_type, prompt, file_url, duration)
        success = False
