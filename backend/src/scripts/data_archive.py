...
class DataArchiver:
    ...
    def archive_old_sessions(self, days: int = 90) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Archive completed | sessions={sessions_count} | "
                f"messages={messages_count} | older_than={days}days"
            )
    ...
    def incremental_backup(self, since: datetime | None = None) -> dict[str, Any]:
        ...
        """
        if not since:
            since = self._stats.get("last_backup_time") or (
        ...
        for table in self.config["tables_to_backup"]:
            ...
            row_count = self._export_table_to_json(
                ...
                output_path=output_file,
                where_clause=(
                    f"t_updated_at >= '{since_str}' OR "
                    f"t_created_at >= '{since_str}'"
                ),
            )
    ...
    def cleanup_old_backups(self, days: int | None = None) -> dict[str, Any]:
        ...
        logger.info(
            f"Cleanup completed | files={result['cleaned_files']} | "
