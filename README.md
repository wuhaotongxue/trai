...
### 💻 客户端_2026_06_03_1413
- **新增(cache)**: 接入 SQLite 本地离线缓存, 支持断网环境历史记录查看
- **优化(ai)**: 视频生成增加 tenacity 退避重试机制, 修复轮询 404 错误
...
- **增强(prompt)**: 重构 `PromptOptimizer` 与图片生成用例, 统一提示词优化入口、输入校验、异常回退与日志记录, 并补充核心单元测试覆盖.
- **优化(frontend)**: 图像编辑面板新增眼镜类快捷指令, 前端轮询阶段信息更细, 并与后端新的状态流转保持一致.
...
- **增强(notify)**: 飞书通知引入“交互式卡片”多重降级机制, 即使未配置 APP_ID 也能通过优化的富文本格式展示高清封面预览, 解决了推送“没图没真相”的痛点.
...
- **feat(tools)**: 优化 B 站视频下载通知逻辑，集成 DeepSeek 河南地理专家身份周五问候与景点随机推荐
- **feat(tools)**: 视频下载支持 S3 存储、历史记录画廊展示、批量复制与删除功能
- **fix(auth)**: 增强游客支持，基于客户端 IP 自动识别并持久化游客对话与下载记录
- **fix(ui)**: 优化下载面板交互，增加 Agent 运行步骤实时展示与进度条
