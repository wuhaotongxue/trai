...
- **Neo-Brutalism / 新粗野主义 (MANDATORY)**: 
  ...
  - **色彩控制：** 避免在一个板块内大面积使用过高饱和度的颜色。主体背景请使用纯白 `bg-white` 或极浅的颜色（如 `bg-slate-50`, `bg-cyan-50`）。高饱和度颜色（如 emerald, indigo, rose, cyan）仅用于按钮、小徽章或边框点缀。
  - **绝对禁止使用黄色/琥珀色 (amber/yellow)**，因为大面积黄色显得难看且刺眼。
...
- **Empty States (空白填充)**: 
  ...
  - 必须使用大尺寸的 Lucide 图标（如 `w-16 h-16`），放在一个带有粗边框和硬核阴影的纯色块背景中（如 `w-32 h-32 bg-cyan-400 rounded-none border-4 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] flex items-center justify-center`）。
