...
- 检查 `md/` 目录，该**根目录**下只能保留**子文件夹**（如 `issue_01` 等）和 `.css` 文件。
  - **严禁删除**子文件夹内部的任何内容（包含子文件夹内的 `.md` 和图片等）。
  - 只有当 `md/` **根目录**下出现散落的 `.md` 或其他杂乱文件时，才在提交前将这些散落文件删除。
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:12px 0;">
  <div style="background:#E3F2FD;border:1px solid #90CAF9;border-radius:8px;padding:12px;">
    ...
    <ul style="margin:8px 0 0 0;padding-left:16px;font-size:13px;color:#555;">
      ...
      <li>修改 <code>frontend_next</code> → 必须调用 <code>frontend_next/SKILL.md</code> 检查 App Router 规范，并执行以下检查(必须零 warning): <code>cd frontend_next; pnpm run lint:fix</code> 与 <code>pnpm run lint -- --max-warnings 0</code></li>
    </ul>
  </div>
