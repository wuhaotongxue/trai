...
| `AWS_REGION` | 是 | S3 区域，如 `ap-northeast-1` |
| `S3_BUCKET_NAME` | 是 | Bucket 名称，默认 `trai-media` |
...
| `WECOM_APP_ID` | 是 | 企业应用 AgentId |
| `WECOM_APP_SECRET` | 是 | 企业应用 Secret |
| `WECOM_API_BASE_URL` | 否 | 企业微信 API 地址，默认 `https://qyapi.weixin.qq.com` |
...
S3_BUCKET_NAME=trai-media

# 企业微信
WECOM_CORP_ID=ww1234567890abcdef
WECOM_APP_ID=1000001
...
|----------|-------------------|------------------------------------------|------------------|
| 会议音视频 | **5 分钟** | **2 小时** | **7 天** |
| 会议聊天记录 | **5 分钟** | **1 小时** | **7 天** |
| 会议文档 | **15 分钟** | **4 小时** | **7 天** |
| 消息附件 | **5 分钟** | **1 小时** | **7 天** |
| 用户头像 | **1 小时** | **12 小时** | **7 天** |
| 临时上传凭证 | **15 分钟** | **30 分钟** | **24 小时** |
...
|-------------|--------|----------------|------|
| 超管（企业创建者） | `admin` | **7 天** | 企业最高管理员 |
| 分级管理员 | `admin` | **7 天** | 可分配权限的管理员 |
...
> 规范要求：角色解析逻辑应委托给 `WeComService`，此处仅定义映射规则，不实现完整类。

**角色映射表**

| 企业微信角色 | S3 角色 | 说明 |
|-------------|--------|------|
| 超管 (superadmin) | `admin` | 企业创建者，最高权限 |
| 分级管理员 (admin) | `admin` | 可分配权限 |
| 部门负责人 (dept_leader) | `dept_leader` | 本部门文件可访问 |
| 普通成员 (member) | `wecom_vip` | 与付费 VIP 同等 |
| 外部访客 (external) | `user` | 受限访问 |
...
实现参考：`backend/src/services/wecom_service.py`

#### 权限流转示例

```
企业微信管理员在后台设置:
├── 超管 / 分级管理员 ──→ S3 role = admin (7天)
...
class S3Client:
    ...
    EXPIRES_CONFIG: dict[str, dict[str, int]] = {
        ...
        # 管理员（超管/分级管理员）— 全部时间权限
        "admin": {
            "meeting_audio": 604800,     # 7 天
            "meeting_video": 604800,     # 7 天
            "meeting_chat": 604800,      # 7 天
            "meeting_doc": 604800,       # 7 天
            "attachment": 604800,        # 7 天
            "avatar": 604800,            # 7 天
            "upload": 86400,             # 24 小时
            "public": 604800,            # 7 天
        },
...
```python
# S3_路径中的__retention_XXX_前缀用于触发不同的_Lifecycle_规则
# 由_Backend_在写入时根据用户角色自动设置前缀
...
```python
# CloudWatch_Events_告警规则_(CloudFormation/YAML)
"""
...
```python
# 每次_S3_操作必须记录结构化日志_(ELK/Grafana_可解析)
logger.info(
    "S3 operation: "
    f"action={action}, "
    f"user_id={user_id}, "
