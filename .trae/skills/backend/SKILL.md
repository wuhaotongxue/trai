...
<div style="background:#FFF3E0;border:1px solid #FFB74D;border-radius:8px;padding:12px 16px;margin:12px 0;">
  <strong style="color:#E65100;">&#x26A0; 沟通与版本表述红线 (CRITICAL)</strong>
  <ul style="margin:8px 0 0 0;padding-left:20px;font-size:13px;">
    <li>本项目<strong>极度严格绑定 Python 3.13.13</strong>。</li>
    <li>在向用户汇报、撰写日志、更新 README 等任何自然语言沟通场景下，<strong>绝对禁止</strong>使用退级的版本号表述（如 <code>3.11+</code>、<code>3.12</code>、<code>3.x</code> 等）。</li>
    <li>即使某个特性（如 <code>StrEnum</code>）是在早期版本引入的，在沟通中也必须且只能统一表述为“符合 3.13.13 规范”，严禁卖弄历史版本常识导致用户产生降级恐慌。</li>
  </ul>
</div>

### 2.1 测试与验证 (MANDATORY)

- 后端接口验证必须写成 Python 测试文件, 统一放在 `backend/tests/` 下, 禁止在终端直接写超长 `python -c "..."` 一次性命令
- 运行测试推荐直接 `python backend/tests/xxx.py` (需确保 trai 环境已激活), 避免使用 `conda run -n trai` (会导致日志无法正常显示)
- 测试脚本默认使用 `http://127.0.0.1:5666` 作为 API_BASE, 不要在命令里拼接复杂引号或环境变量
- PowerShell 多条命令用 `;`, 不要用 `&&`
- 测试用例仅用于验证当前需求, 验证通过后若无长期价值应删除, 禁止遗留临时测试文件

### 2.2 README 与 Changelog (MANDATORY)

- 修改 `backend/` 相关代码后, 必须同步更新两处 Changelog: 根目录 `README.md` 与 `backend/README.md`
...
### 4.1 OpenAPI 文档中文化规范 (MANDATORY)

- Router 装饰器必须补齐 `summary` 与 `description`, 且用中文描述
- Pydantic `Field(description=...)` 必须用中文描述, 且覆盖所有对外字段
- `tags` 必须使用中文分组名, 保持与 `api/main.py` 中 `openapi_tags` 一致
- 禁止依赖函数名自动生成文档, 否则 `/docs` 会出现 `SendNotify` `MdToPdfEndpoint` 这类英文
- 中文内容允许, 但必须遵守中文标点禁令, 禁止使用全角标点, 统一使用半角 `, . : ? !`
