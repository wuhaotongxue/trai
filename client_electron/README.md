...
- **优化(icon)**: AI对话页面Agent列表图标改为Cpu，与侧边栏Agent图标保持一致
...
### 💻 客户端_2026_04_19_1710
- **优化(agent)**: Agent 管理页面布局优化，图标缩小至36px，标题字体统一为14px
- **优化(ui)**: Agent 详情描述和系统提示词只显示4个字+省略号，移除重复名称显示
- **优化(ui)**: Agent 列表字体统一为14px与操作面板一致
- **优化(ui)**: 右侧内容区添加省略号处理并减小padding
