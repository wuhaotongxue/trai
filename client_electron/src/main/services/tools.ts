...
export const tools_service = {
   ...
   * @returns 转换结果
   */
  async convert_word_to_pdf(file_path: string) {
    try {
      const url = ApiUrl.build_api_url(ApiEndpoints.tools_word_to_pdf)
      const form_data = new FormData()
      ...
      log.error('convert_word_to_pdf failed:', error.response?.data || error.message)
      return { 
        success: false, 
        error: error.response?.data?.detail?.message || 'Word 转 PDF 失败' 
      }
    }
  },
   ...
   * @returns 转换结果
   */
  async convert_pdf_to_word(file_path: string) {
    try {
      const url = ApiUrl.build_api_url(ApiEndpoints.tools_pdf_to_word)
      const form_data = new FormData()
      ...
      log.error('convert_pdf_to_word failed:', error.response?.data || error.message)
      return { 
        success: false, 
        error: error.response?.data?.detail?.message || 'PDF 转 Word 失败' 
      }
    }
  },
   ...
   * @param target_format - 目标格式 (csv, json, xlsx)
   * @returns 转换结果
   */
  async convert_excel(file_path: string, target_format: string) {
    try {
      const url = ApiUrl.build_api_url(ApiEndpoints.tools_convert_excel)
      const form_data = new FormData()
      ...
      log.error('convert_excel failed:', error.response?.data || error.message)
      return { 
        success: false, 
        error: error.response?.data?.detail?.message || 'Excel 转换失败' 
      }
    }
  }
}
